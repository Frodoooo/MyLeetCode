#定义s[j]向上拨动一次
def switchup(mystr, j):
    if (mystr[j] == '9'):
        newstr = mystr[0:j] + '0' + mystr[j + 1:]
    else:
        newstr = mystr[0:j] + str(int(mystr[j]) + 1) + mystr[j + 1:]
    return newstr

#定义s[j]向下波动一次
def switchdown(mystr, j):
    newstr = mystr
    if (mystr[j] == '0'):
        newstr = mystr[0:j] + '9' + mystr[j + 1:]
    else:
        newstr = mystr[0:j] + chr(ord(mystr[j]) - 1) + mystr[j + 1:]
    return newstr
    ...
    #BFS框架
        ...
        while(q1 and q2):
            ...
            for i in range(size):
                ...
                if (cur in q2):return move
                #将每个节点的未遍历相邻节点加入队列
                for j in range(len(mystr)):
                    q1.append(switchup(cur,j))
                    q1.append(switchdown(cur,j))
                visited.append(cur)
            #增加步数
            temp = q1[:]
            q1 = q2[:]
